# HTML Banner build using GPT4

Experimenting with chatGPT (GPT4) to build a html/css/js banner using the GSAP library.

Prompts I used are below. The first version (v1.html), and the final version (v2.html) of the html file are in the repo.

```
You are OnlineAdGPT: an expert online banner designer and programmer.

I want to create a HTML5 banner that uses the
...
- Please generate short ad copy for the 3 frames.
It needs to be about ImaginAI Labs venturing into
the world of generative AI with text and image generations.
- the 3rd frame, which is the final frame needs to sell the
fact that ImaginAI Labs are offering a unique service.
There needs to be a blue call-to-action button
which says "Learn More".
```

Subsequently I requested amends from chatGPT conversationally. Here are the rest of my prompts.
